...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("Work in Progress: Our database and servers are undergoing cleaning. Registration is temporarily disabled.");
  };
  ...
  return (
    <div className="min-h-screen hero-gradient circuit-bg flex items-center justify-center px-4 py-12">
      ...
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex flex-col items-center gap-2">
            ...
            <p className="text-brand-silver/60 text-xs tracking-widest">CREATE AN ACCOUNT</p>
          </Link>
        </div>

        <div className="navy-glass rounded-2xl p-8">
          <h1 className="font-bebas text-white text-3xl tracking-wide mb-1">REGISTER</h1>
          <p className="text-brand-silver/60 text-sm mb-6">Create your client portal account to track your documents</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            {[
              { name: "name", label: "Full Name", type: "text", placeholder: "Juan dela Cruz", icon: User },
              { name: "email", label: "Email Address", type: "email", placeholder: "juan@example.com", icon: Mail },
              { name: "phone", label: "Phone Number", type: "tel", placeholder: "09XX-XXX-XXXX", icon: Phone },
            ].map((field) => (
              <div key={field.name}>
                ...
                <div className="relative">
                  <field.icon size={16} className="absolute left-3.5 top-1/2 -translate-y-1/2 text-brand-silver/40" />
                  <input
                    name={field.name}
                    type={field.type}
          ...
          <p className="text-xs text-brand-silver/30 text-center mt-4">
            By registering, you agree to our{" "}
            <Link href="/terms" className="text-brand-royal hover:underline">Terms</Link> and{" "}
            <Link href="/privacy" className="text-brand-royal hover:underline">Privacy Policy</Link>.
          </p>

          <p className="text-center text-brand-silver/50 text-sm mt-4">
            Already have an account?{" "}
            <Link href="/login" className="text-brand-royal font-semibold hover:underline">Sign in</Link>
          </p>
        </div>
